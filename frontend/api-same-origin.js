// Обновленный api.js для работы через Cloudflare Worker proxy
// Теперь все запросы идут к тому же домену (same-origin)

// API_URL теперь относительный - все запросы идут к тому же домену
const API_URL = window.location.hostname === 'localhost'
    ? 'http://localhost:3000/api'
    : '/api'; // ✅ ИЗМЕНЕНО: относительный путь вместо полного URL
...
async function apiRequest(endpoint, options = {}) {
    const defaultOptions = {
        ...
        credentials: 'include', // ✅ Теперь работает с same-origin
    };
