const Dashboard = (() => {
    ...
    function switchSection(section) {
        ...
        // Portfolio section
        document.getElementById('portfolio-section').style.display = section === 'portfolio' ? '' : 'none';
...
const PortfolioAdmin = (() => {
    ...
    // ── Load & render table ────────────────────────────
    async function load() {
        const tbody = document.getElementById('pf-admin-tbody');
        const empty = document.getElementById('pf-admin-empty');
        if (!tbody) return;
        if (empty) empty.style.display = 'none';
        tbody.innerHTML = `<tr><td colspan="7" style="padding:2rem;text-align:center;color:rgba(255,255,255,.3)">Ładowanie…</td></tr>`;

        const platform = document.getElementById('pf-filter-platform')?.value || 'all';
        const plan     = document.getElementById('pf-filter-plan')?.value     || 'all';

        try {
            const data = await API.adminListPortfolio(platform, plan);
            ...
            data.projects.forEach(p => {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td class="td-id" style="font-family:var(--font-mono)">#${p.id}</td>
                    <td style="font-weight:500">${esc(p.title)}</td>
                    <td><span class="badge pf-platform-badge pf-platform-badge--${p.platform}">${esc(p.platform)}</span></td>
                    <td><span class="badge priority-normal">${esc(p.plan)}</span></td>
                    <td>${esc(p.price || '—')}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="7" style="padding:2rem;text-align:center;color:var(--accent-urgent);">${t('tickets_load_error')}: ${esc(err.message)}</td></tr>`;
        }
    }
    ...
    async function openEdit(id) {
        try {
            ...
            showToast(t('tickets_load_error') + ': ' + err.message, 'error');
        }
    }
    ...
    async function deleteProject(id) {
        ...
        try {
            ...
            showToast(t('admin_portfolio_deleted'), 'success');
            await load();
        } catch (err) {
            showToast(t('tickets_load_error') + ': ' + err.message, 'error');
        }
    ...
    function setupForm() {
        document.getElementById('pf-form').addEventListener('submit', async e => {
            ...
            try {
                ...
                if (editId) {
                    await API.adminUpdatePortfolioProject(parseInt(editId), payload);
                    showToast(t('admin_portfolio_updated'), 'success');
                } else {
                    await API.adminCreatePortfolioProject(payload);
                    showToast(t('admin_portfolio_created'), 'success');
                }
                ...
                submitBtn.textContent = pfEditId ? t('promo_btn_save') : t('admin_portfolio_btn_create');
            }
        });
    }
    ...
    // ── Filter change ──────────────────────────────────
    function setupFilters() {
        ['pf-filter-platform', 'pf-filter-plan'].forEach(id => {
            const el = document.getElementById(id);
